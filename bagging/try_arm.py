...
def arm_move(mode='PTP', speed=100 ,acceleration=50):
    global Point_now
    global Point_Dist
    if mode=='PTP':
...
# Move PTP
def PTP_Move(Point, speed=100 ,acceleration=50):
...
def Angel_Move(Point, speed=100 ,acceleration=50):
    print("=============Angel==================")
    C_PTP_XYZ = (c_double * len(Point))(*Point)         # C Array
...
# Move LIN   
def LIN_Move(Point, speed=100 ,acceleration=50):
    print("    LIN Move ... " )
...
if __name__ == "__main__":
    ...
    arm_move('PTP', speed=100 ,acceleration=50)
    ...
    while 1:
        img = get_picture()
        yolo_info,img2,det= YOLO_Detect.detect_ALL(img)
        if det == 0:
            time.sleep(0.5)
            continue
        print("yolo_info= ",yolo_info)  
        yolo_info,img3,det = YOLO_Detect.mouse_ALL(img2)
        if det == 0:
            time.sleep(0.5)
            continue
        print("yolo_info= ",yolo_info)
    ...
    Point_Dist = [ To_Mouse_x, To_Mouse_y, 20, -180.000, 0 ,90.000]
    arm_move('PTP', speed=100 ,acceleration=50)
